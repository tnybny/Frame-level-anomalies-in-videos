...
class Experiment(object):
    def __init__(self, tvol, alpha, batch_size, lambd=0.0, gamma=0.0):
        self.tvol = tvol
        ...
        self.vel_reg = tf.reduce_sum(self.y[:, :, :, 1:] - self.y[:, :, :, :-1])

        self.loss = self.reconstruction_loss + lambd * self.regularization_loss + gamma * self.vel_reg
